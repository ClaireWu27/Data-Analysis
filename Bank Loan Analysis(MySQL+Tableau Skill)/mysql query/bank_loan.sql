...
select year(issue_date) as year, month(issue_date) as month, monthname(issue_date) as month_name, count(id) as total_loan_application, sum(loan_amount) as total_funded_amount, sum(total_payment) as total_amount_received
from financial_loan 
group by 1,2,3
order by 1,2;
