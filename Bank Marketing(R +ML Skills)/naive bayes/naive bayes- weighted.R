...
set.seed(123)  
index <- sample(1:nrow(cleaned_data), round(0.9 * nrow(cleaned_data)))
train_data <- cleaned_data[index, ]
test_data <- cleaned_data[-index, ]

# Calculate class probabilities based on the training data
class_probs <- table(train_data$y) / nrow(train_data)

# Training the Model with adjusted class probabilities
model <- naiveBayes(y ~ ., data = train_data, laplace = 1, apriori = class_probs)
...
model <- naiveBayes(y ~ age + education + contact + month + day_of_week + 
                    ...
                    data = train_data,apriori = class_probs)
